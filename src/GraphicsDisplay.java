...
public class GraphicsDisplay extends JPanel {
    ...
    public void paintComponent(Graphics g) {
        ...
        if (isRotated) {
            ...
            tform.quadrantRotate(-1, (maxY - minY) / 2 * scale, (maxX - minX) / 2 * scale);
            tform.translate((maxY - minY) / 2 * scale-(maxX - minX) / 2 * scale, (maxX - minX) / 2 * scale-(maxY - minY) / 2 * scale);
    ...
    protected Double[] getMinMax(Double[][] graphicsData) {
        ...
        /*
         * if(isRotated) { double minXR = minmaxxy[2]; double maxXR = minmaxxy[3];
         * double minYR = minmaxxy[0]; double maxYR = minmaxxy[1];
         *
         * minmaxxy[0] = minXR; minmaxxy[1] = maxXR; minmaxxy[2] = minYR; minmaxxy[3] =
         * maxYR; }
         */
    ...
    protected void paintAxis(Graphics2D canvas) {
        ...
        if ((minX <= 0.0 && maxX >= 0.0) || (minY <= 0.0 && maxY >= 0.0 && isRotated)) {
            drawY(canvas);
        }
        if ((minY <= 0.0 && maxY >= 0.0) || (minX <= 0.0 && maxX >= 0.0 && isRotated)) {
            drawX(canvas);
        }
    }
    ...
    protected void drawY(Graphics2D canvas) {
        ...
        Point2D.Double lineStart = xyToPoint(0, minY);
    ...
    protected void drawX(Graphics2D canvas) {
        ...
        Point2D.Double lineStart = xyToPoint(minX, 0);
